...
export default function AdminLayout() {
  ...
  return <div className="min-h-screen flex bg-muted/30">
      {/* Sidebar */}
      <aside className="hidden lg:flex w-72 flex-col bg-sidebar-background text-sidebar-foreground border-r border-sidebar-border fixed inset-y-0 left-0 z-40">
        {/* Brand */}
        <div className="p-6 border-b border-sidebar-border">
          ...
          <Link to="/" className="flex items-center gap-1.5 text-xs text-sidebar-foreground/50 hover:text-primary transition-colors">
        ...
        <nav className="flex-1 p-4 space-y-1">
          <p className="text-[10px] uppercase tracking-widest text-sidebar-foreground/30 font-semibold px-3 mb-2">Navigation</p>
          {navItems.map(item => {
          const active = location.pathname === item.path;
          return <Link key={item.path} to={item.path} className={`flex items-center gap-3 px-3 py-2.5 rounded-lg text-sm font-medium transition-all duration-200 ${active ? "bg-primary/15 text-primary shadow-sm" : "text-sidebar-foreground/60 hover:text-sidebar-foreground hover:bg-sidebar-accent/50"}`}>
        ...
        <div className="p-4 border-t border-sidebar-border">
          <div className="flex items-center gap-3 px-3 py-2">
            ...
            <div className="flex-1 min-w-0">
              ...
              <p className="text-xs text-sidebar-foreground/40 truncate">admin@goldvault.in</p>
            </div>
            <button className="p-1.5 text-sidebar-foreground/40 hover:text-sidebar-foreground transition-colors">
